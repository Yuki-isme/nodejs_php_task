const Model_Ignore = {
    ...
    __init: async (model, defaultFieldDef = true, actionFieldDef = true) => {
        if (actionFieldDef) {
            await model.fieldDefs.unshift(...Model_Ignore.actionFieldDef);
        }
        if (defaultFieldDef) {
            await model.fieldDefs.unshift(...Model_Ignore.defaultFieldDef);
        }

        model.fieldDefs = await Model_Ignore.setIndex(model.fieldDefs);
    ...
    buildQueryGetRecords: async (req, model, type) => {
        ...
        await model.fieldDefs.forEach((field) => {
            ...
            if (field.onlySelect) {
                ...
                select += `${Model_Ignore.generateActionHTML(req, model)} AS action`;
            } else {
                // console.log(`\r\n${Model.generateFieldHTML(req, model, field)}`);
                select += Model_Ignore.generateFieldHTML(req, model, field);
                if (field.relation === false) {
    ...
    getRecords: async (req, res, model) => {
        const [query, queryCount] = await Model_Ignore.buildQueryGetRecords(req, model, 'listing');
    ...
    getFullRecords: async (req, res, model) => {
        const [query] = await Model_Ignore.buildQueryGetRecords(req, model);
        const [results] = await pool.query(query);
        return results;
    },
